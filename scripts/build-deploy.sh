...
cd "$REPO_ROOT"
pnpm turbo build --filter=@edforge/shell --filter=@edforge/academics --filter=@edforge/people --filter='./packages/*' --filter='./types/packages/*'
...
REMOTES=(academics people)
# [MVP-PARKED] Parked modules excluded from deployment
# REMOTES_PARKED=(edfi special-programs messages analytics finance)
# [/MVP-PARKED]
...
PARKED_REMOTES=(edfi special-programs messages analytics finance)
