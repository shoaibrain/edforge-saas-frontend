...
| GF0.2 | Scaffold `packages/archetype` (`@edforge/archetype`): `package.json`, `tsconfig`, `vitest.config`, `src/index.ts`; add to `pnpm-workspace.yaml` + `turbo.json` (both confirmed present). **`@edforge/archetype` is workspace-private, shell/MFE-only — it must never be imported by AdminWeb** (separate repo, CRA/webpack, workspace-only-in-Docker trap per CLAUDE.md). | `pnpm install && pnpm build --filter @edforge/archetype` succeeds; package marked private. |
...
| GF0.6 | Author **inert** `CBSE_IN`/`NAIS_US`/`GEMS_UAE` stubs identical to `GENERIC` — these are *type-completeness* placeholders for the reserved enum, **not runtime-selectable** (backend `activeArchetypeSchema` is `['PABSON','GENERIC']` only; CLAUDE.md). | Unit test: each resolves to GENERIC output without throwing; **a test asserts none of the three is in the runtime-active set** (mirrors the GF5.5 prod-guard discipline), so they can't be picked in the UI. |
...
> **GF3.4 remains** — the read-only "Governance profile" panel in settings (new
> component, needs its own route→component trace + visual smoke). GF3.5 (i18n
> labels) rides with GF3.4 once the panel introduces operator-facing label keys.
...
| GF5.3 | **The proof ticket.** Add `'CBS'` to a test-only active set and run the GF0 conformance + identifier suites against it. Prove ZERO app edits via a **real, repeatable CI gate** (not eyeballing), mirroring backend GB4.3. | A committed `scripts/lint/assert-archetype-data-only.sh` runs `git diff --name-only <base>..HEAD` and fails non-zero if any path matches `^apps/`. Conformance + identifier suites green for CBS; the script is wired into CI and is the headline acceptance gate. |
| GF5.4 | Refresh the "Adding a governance body (frontend)" guide (GF0.10): **generate its CBS file list from the actual GF5 `git diff --name-only`** so it can't drift, and document the coordinated-with-GB4 publish handshake. | Guide's file list is generated (not hand-written) and matches the GF5 diff; reviewer confirms. |
