...
const LOCATION_TYPE_ICONS: Record<string, string> = {
  classroom: 'bg-teal-500/10 text-teal-600',
  lab: 'bg-purple-500/10 text-purple-600',
  gym: 'bg-green-500/10 text-green-600',
  auditorium: 'bg-amber-500/10 text-amber-600',
  library: 'bg-indigo-500/10 text-indigo-600',
      ...
      {locations.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {Object.entries(typeBreakdown).map(([type, count]) => (
            <span
              key={type}
              className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${LOCATION_TYPE_ICONS[type] || 'bg-gray-100 text-gray-600'}`}
            >
      ...
      {locations.length > 5 && (
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 text-sm rounded-xl border border-[rgb(var(--border-primary))] bg-[rgb(var(--surface-secondary))] focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-500"
          />
        </div>
      )}
          ...
          <motion.div variants={staggerChildren} initial="initial" animate="animate">
            {filteredLocations.map((loc) => (
              ...
              >
                ...
                <div className="col-span-2">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${LOCATION_TYPE_ICONS[loc.locationType] || 'bg-gray-100 text-gray-600'}`}>
                ...
                <div className="col-span-1">
                  <span className={`inline-flex items-center gap-1 text-xs font-medium ${loc.isActive ? 'text-teal-600' : 'text-[rgb(var(--text-tertiary))]'}`}>
                    <span className={`w-1.5 h-1.5 rounded-full ${loc.isActive ? 'bg-teal-500' : 'bg-gray-400'}`} />
