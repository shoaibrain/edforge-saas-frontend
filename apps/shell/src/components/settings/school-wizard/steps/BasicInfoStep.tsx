...
export function BasicInfoStep({ data, updateData, errors, clearError }: WizardStepProps) {
  ...
  // Sprint C Gap 3 / GF3.3 — required-ness comes from the GF3.1 feature matrix
  // (`fieldRequirement`), not a hardcoded archetype check, so a future
  // governance body that requires the field is a data change. `isPabson` is
  // kept only for the PABSON-specific IEMIS *copy* (label/placeholder/help).
  const { archetype } = useTenant()
  const isPabson = archetype === 'PABSON'
  const emisRequired = fieldRequirement('emisSchoolCode', archetype) === 'required'
