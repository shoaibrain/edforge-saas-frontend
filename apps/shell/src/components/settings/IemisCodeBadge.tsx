 ...
 * - Green-tinted "verified" badge when the code matches the 8–10 digit format
 * - Amber chip with AlertCircle icon when the code is present but malformed
 * - Nothing rendered at all when the school has no emisSchoolCode
 *   (non-PABSON tenants — chip is truly optional)
 * - Copy-to-clipboard button + tooltip explaining CEHRD registry + immutability
 */
 ...
 *
 * Backend `isValidIemisSchoolCode` ships in `@aibrains/shared-types` 0.31.0+.
 * The frontend currently pins `^0.29.0` across all apps, so the canonical
 * import is not yet resolvable. Duplicated here to keep the format check
 * aligned without blocking this UI ship on a cross-cutting pin bump.
 * Once the frontend pin bumps to `^0.32.0` (required anyway for the Sprint 3
 * Demographics tab), replace this with the canonical import. Behavior must
 * stay identical: ASCII digits only, length 8–10.
 */
