/**
 * BulkReceiptPdfExportDrawer — Sprint G.4.
 *
 * Operator-facing progress drawer for the G.2 bulk-receipt-PDF-export
 * worker. Structural mirror of `BulkPdfExportDrawer` (F.5) with the
 * invoice-side hook + i18n keys swapped for the receipt-side. Sibling-
 * drawer conformance is identical.
 *
 * Note on eligibility: unlike `BulkSendReceiptsDrawer` (which computes
 * eligibility client-side and shows a split of "will send / will skip"),
 * this drawer sends all selected paymentIds to the backend and lets the
 * G.2 worker's eligibility filter run (only status='completed' payments
 * have receipts). Non-completed payments come back as `counters.skipped`,
 * NOT `counters.failed` — the AsyncJobProgress "summary" line already
 * distinguishes these three counts cleanly. No client-side eligibility
 * pre-filter is needed and would just re-implement backend logic.
 *
 * Flow:
 *   1. Drawer opens with pre-kickoff summary: "Export N receipts as ZIP"
 *      + a Start-export button.
 *   2. Start-export → POST /finance/schools/:schoolId/payments/bulk-pdf-export
 *      via `useBulkReceiptPdfExport`. Backend returns 202 + {jobId, message}.
 ...
 *     flat paymentIds[]; there's no per-row eligibility branch)
 ...
 *
 * Backend contract:
 *   - `useBulkReceiptPdfExport` wraps POST bulk-pdf-export (F.4)
...
export function BulkReceiptPdfExportDrawer({
  ...
  // Toast on terminal — once. Also invalidate the payments list on
  ...
  useEffect(() => {
    ...
    if (job.data.status === 'succeeded') {
      toast.success(
        t('asyncJobs.receiptPdfExport.toastSucceeded', {
      ...
      toast.error(t('asyncJobs.receiptPdfExport.toastFailed'))
      setTerminalLogged(true)
    }
    ...
    try {
      ...
      if (getErrorStatus(err) === 409) {
        ...
        if (body?.runningJobId) {
          ...
          toast.info(t('asyncJobs.receiptPdfExport.alreadyRunning'))
          return
        }
      }
      ...
      if (getErrorStatus(err) === 413) {
        ...
        toast.error(
          t('asyncJobs.receiptPdfExport.payloadTooLarge', {
            limit: body?.limit ?? 2000,
            requested: body?.requested ?? paymentIds.length,
          }),
        )
        onClose()
        return
      }
      toast.error(err instanceof Error ? err.message : t('asyncJobs.receiptPdfExport.toastFailed'))
    }
  }
    ...
    <AnimatePresence>
      {open && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="bulk-receipt-pdf-export-title"
        >
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
              ...
              <div className="flex h-full flex-col bg-[rgb(var(--background-primary))] shadow-xl border-l border-[rgb(var(--border-primary))]">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-[rgb(var(--border-primary))]">
                  ...
                  <button
                    ...
                    data-testid="bulk-receipt-pdf-export-close"
                  >
                    <X className="w-5 h-5" />
                  </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
                  {showProgress ? (
                    <>
                      <AsyncJobProgress
                        job={adaptedJob}
                        verbingNoun={t('asyncJobs.receiptPdfExport.progress')}
                      />
                      {canDownload && (
                        <a
                ...
                <div className="shrink-0 flex items-center justify-end gap-3 px-6 py-4 border-t border-[rgb(var(--border-primary))] bg-[rgb(var(--background-secondary)/0.5)]">
                  <button
                    ...
                    data-testid="bulk-receipt-pdf-export-cancel"
                  >
                  ...
                  {!showProgress && (
                    <button
                      type="button"
                      onClick={handleStart}
                      disabled={isKickingOff || paymentIds.length === 0}
                      ...
                      data-testid="bulk-receipt-pdf-export-start"
                    >
                      {isKickingOff
                        ? t('asyncJobs.receiptPdfExport.starting')
                        : t('asyncJobs.receiptPdfExport.startExport')}
                    </button>
                  )}
                </div>
              </div>
