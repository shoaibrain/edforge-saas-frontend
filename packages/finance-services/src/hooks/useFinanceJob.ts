...
export function useFinanceJob(jobId: string | null | undefined) {
  ...
  return useQuery<FinanceJobRow, Error>({
    ...
    select: (data) => {
      // On terminal-succeeded, invalidate the invoice list so the
      // operator sees fresh data when they close the modal (some
      // invoices may have been marked, audit emits have fired, etc.).
